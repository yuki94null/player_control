# タイマーではじく
execute unless score $WallRunTime yrh.010.global matches -1 if score @s yrh.010.wall_run.time > $WallRunTime yrh.010.global run return fail
execute unless score $TotalWallRunTime yrh.010.global matches -1 if score @s yrh.010.wall_run.time.total > $TotalWallRunTime yrh.010.global run return fail
